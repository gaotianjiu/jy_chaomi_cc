...
class MainController extends BaseController {
    ...
    function actionIndex() { //首页 
        ...
        //------行情列表-----end
        //------公告-----begin
        $pub_announc = new pub_announce(); //++
        $announce_ret = $pub_announc->findAll(array("status" => 0), "id desc"); //++
        //--$announce_ret = new pub_announce()->findAll("status=0", "id desc");
        unset($announce_ret['content']);
        unset($announce_ret['introduction']);
        for ($i = 0; $i < count($announce_ret); $i++) { //++ //++
            $announce_ret[$i]['update_time'] = date("Y/m/d", strtotime($announce_ret[$i]['update_time']));  //++
    ...
    function actionf() { //单个品种页
        ...
        //------公告-----begin
        $gb = new pub_announce();
        $announce_ret = $gb->findAll("status=0", "id desc");
        unset($announce_ret['content']);
        unset($announce_ret['introduction']);
        $announce_ret['update_time'] = date("Y/m/d", strtotime($announce_ret[0]['update_time']));
        $this->announce_ret = $announce_ret[0];
        //------公告-----end		
        $this->pz = $pz;
        $this->type_name = $pz;
    ...
    function actionwt_price() {//单个品种右则委托价格输出
        $typeid = intval($this->spArgs('typeid'));
        ...
        if (empty($typeid))
            json_s(array('status' => 200, 'msg' => 'typeid不能为空'));
        if ($type == 'box') {
            $types = check_pz($typeid);
            $pz = $types[0]['name']; //品种名称			
            $this->type_name = $pz;
        ...
        if ($type == 'all')
            json_s(array('status' => 200, 'buy_list' => find_buy_sale_all($typeid, 1, 100, 0, 0), 'sale_list' => find_buy_sale_all($typeid, 0, 100, 0, 0), 'hq_list' => $hq_data));
