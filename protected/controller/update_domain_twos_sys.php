...
class update_domain_twos_sys extends spController
{
	...
	function create(){	
		...
		// exit('free...');
		$tg_typeid_data = array(808001,808008,808018);
		...
		if($typeid==808001){
			$typeid_cond_sql = 'typeid in (808001,808009,808010,808011,808012,808013,808014,808015,808016,808017)';
		}
		$skip_uid = array(19668,19639);//跳过指定的UID
		if(!$r){
...
function domain_twos_work($typeid,$uid){	
	...
	$typeid_cond_sql = "typeid={$typeid}";
	if($typeid==808001){
		// $typeid_cond_sql = 'typeid>=808001 and typeid<=808017';
		$typeid_cond_sql = 'typeid in (808001,808009,808010,808011,808012,808013,808014,808015,808016,808017)';
	}	
